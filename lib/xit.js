...
function start () { // {{{
	...
	book.has_xid = function (serial) {
		logwrap.debug( 'Checking for '.concat( xact.serial, ' in book (has_xid).' ) );
		if (book.hasOwnProperty( serial )) {
			logwrap.debug( 'Found '.concat( xact.serial, ' in book (has_xid).' ) );
			return true;
		}
		logwrap.debug( 'Didn\'t find '.concat( xact.serial, ' in book (has_xid).' ) );
		return false;
	}
...
function add_xact (xact) { // {{{
	var serial_lookup = book.get( xact.serial );

	logwrap.debug( 'Adding xact id '.concat( xact.serial ) );

	if (serial_lookup != undefined) {
		return logwrap.error( 'Requested new transaction ID '.concat( xact.serial, ' but this is an extant transaction.' ) );
	}
	if (serial_lookup == undefined) {
		logwrap.debug( 'New serial '.concat( xact.serial, ' encountered, adding.' ) );
	}
	if (xact.state === 'closed') {
		return logwrap.error( 'Requested new transaction ID '.concat( xact.serial, ' but this transaction happened in the past and is now closed.' ) );
	}
	...
	logwrap.debug( 'Transaction ID '.concat( xact.serial, ' opened with stamp ', xact.opened ) );
...
function end_xact (serial) { // {{{
	if (book.has_xid( serial )) {
		...
		else if (book.get_xact(serial).state != 'open') {
...
function nonce () { return require('crypto').randomBytes(Math.ceil(16)).toString('hex') }

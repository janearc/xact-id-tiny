...
function start () { // {{{
	...
	book.get = function (serial) {
		...
		//
		logwrap.debug( 'Checking for '.concat( serial, ' in book (get).' ) );
...
function add_xact (xact) { // {{{
	var serial        = xact.serial
		, serial_lookup = book.get( xact.serial );
...
function end_xact (xact) { // {{{
	...
	if (book.has_xid( serial )) {
		...
		else if (book.get(serial).state != 'open') {
	...
	logwrap.debug( 'Dutifully closed '.concat( serial, ' at ', x.closed ) );
